...
@Composable
fun AddTaskToTemplateScreen(
    viewModel: DayTemplateViewModel,
    dayTemplateId: String,
    onNavigateBack: () -> Unit
) {
    // Получаем имя шаблона дня, чтобы отобразить его в заголовке
    val dayTemplateWithTasks by viewModel.getTemplateById(dayTemplateId).collectAsState(initial = null)
    val dayTemplateName = dayTemplateWithTasks?.template?.name ?: "..."
    ...
    val isFormValid = title.isNotBlank() && (startTime == null || endTime == null || !startTime!!.isAfter(endTime))

    Scaffold(
        topBar = {
            TopAppBar(
                title = { Text("Новая задача в шаблон") },
                navigationIcon = {
        ...
        Column(
            ...
            TaskFormContent(
                ...
                headerText = "Добавить в \"$dayTemplateName\"",
                title = title,
            ...
            Button(
                onClick = {
                    viewModel.addTaskToDayTemplate(
